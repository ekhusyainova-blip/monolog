# B_papka/C_file.py
# Базовый интерфейс — решение.
# Берёт условия и события из B файла, формирует решение.
...
# [B] b_fragment — условия и события (приняты из B файла)
def b_fragment(data):
    return {
...
# [C] c_fragment — решение (формируется здесь)
def c_fragment(data):
    return {
