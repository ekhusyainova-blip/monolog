# interpret_chat.py — интерпретация чата AI Monolog Chat
# Слой B. Выбирает провайдера/ключа, собирает контекст, готовит пакет для solve_chat.
# Держит EVENT-шину (emit/SEND) — внутреннюю и внешнюю.

import time
from data_chat import PROVIDERS, KEYS, PROMPTS, SETTINGS, MESSAGES

# ---------- ШИНА СОБЫТИЙ ----------
# emit — внутренняя шина (между слоями).
# SEND — внешняя шина (в EVENTS, потом во фронт через SSE).
...
# ---------- ВЫБОР КЛЮЧА ----------
# Ротация: живой ключ с самым старым last_used.
